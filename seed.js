...
async function executeSeeds() {
  ...
  await db.setting.insert({ key: "site_name", type: 0, value: "Createdby" });
  await db.setting.insert({
    key: "site_logo",
    type: 0,
    value: "https://createdby.co/assets/img/logo.png",
  });
  ...
  await db.setting.insert({
    key: "copyright",
    type: 0,
    value: "Copyright © 2021 Createdby. All rights reserved.",
  });
  await db.setting.insert({
    key: "license_key",
    type: 4,
  ...
  await db.rule.insert({
    output_variable_name: "Age",
    actives: '["Camomile"]',
    operator: 3,
    compare_value: "30",
    action: 2,
  });
  await db.quiz.insert({
    name: "The Basics",
    description: "Let’s start by getting to know you.",
  });
  await db.quiz.insert({
    name: "Your Skin",
    description: "Let’s learn about your skin and it’s uniqueness.",
  });
  await db.quiz.insert({
    name: "Your Environment",
    description: "Almost done, let’s talk environment & lifestyle.",
  });
  ...
  await db.output_variable.insert({
    ...
    priority: 0,
    ranges_response:
      '[{"0-33":"Your skin is not that sensetive"},{"34-66":"Your skin is somewhat sensetive"},{"67-100":"Your skin is highly sensetive"}]',
  });
  await db.output_variable.insert({
    name: "Age",
    priority: 70,
  ...
  await db.question.insert({
    ...
    weight: 100,
    question:
      "How often do you have breakouts, such as an occasional pimple or frequent breakouts (such as chronic acne)?",
  });
  await db.question.insert({
    order: 4,
    id: 9,
    ...
    extra_output_variable:
      '[{"name":"Sun","weight":10},{"name":"Hyperpigmentation","weight":10}]',
  ...
  await db.question.insert({
    order: 1,
    question:
      "Let’s adjust your cream to your geography. Enter your city below",
    placeholder: "city",
    id: 21,
    quiz_id: 3,
    target: 4,
  ...
  await db.question.insert({
    ...
    weight: 100,
    question:
      "How many hours a day do you sit in front of a screen, ex phone, computer, tv etc.?",
  });
  await db.question.insert({
    order: 7,
    id: 27,
  ...
  await db.answer.insert({
    ...
    answer: "<25",
    response_body:
      "We will focus on actives that will improving your skin health and appearance without adding anything unnecessary.",
  });
  await db.answer.insert({
    order: 2,
    question_id: 2,
  ...
  await db.answer.insert({
    ...
    answer: ">40",
    response_body:
      "We have so many luxurious actives that will work to address any of the desires that come with time and knowledge.",
  });
  await db.answer.insert({
    order: 1,
    question_id: 3,
  ...
  await db.answer.insert({
    ...
    response_header: "Noted.",
    response_body:
      "We have many wonderful ingredients that are specifically proven to be soothing for sensitive skin. We will add these along with other functional actives that are non-sensitizing.",
  });
  await db.answer.insert({
    order: 1,
    question_id: 7,
  ...
  await db.answer.insert({
    ...
    response_body:
      "We have some wonderful ingredients that will maintain optimal skin hydration while controlling excess oil production.",
  });
  await db.answer.insert({
    order: 3,
    question_id: 7,
    ...
    response_body:
      "We will make sure to add actives that will help control excess skin oiliness, while still addressing all your other skin needs.",
  });
  await db.answer.insert({
    order: 1,
    question_id: 8,
  ...
  await db.answer.insert({
    ...
    response_header: "Great!",
    response_body:
      "We'll make sure not to add any actives that may lead to breakouts in breakout-prone skin",
  });
  await db.answer.insert({
    order: 3,
    question_id: 8,
  ...
  await db.answer.insert({
    ...
    answer: "Never",
    response_body:
      "Keep the sunscreen close! We’ll make sure to add some wonderful actives that will help you with your beautiful glow",
  });
  await db.answer.insert({
    order: 2,
    question_id: 10,
    answer: "Sometimes",
    response_body:
      "Don’t forget the sunscreen! We’ll make sure to add some wonderful actives that will help you with your beautiful glow",
  });
  await db.answer.insert({
    order: 3,
    question_id: 10,
    answer: "Always",
    response_body:
      "We’ll make sure to add some wonderful actives that will help you with your beautiful glow.",
  });
  await db.answer.insert({
    order: 2,
    question_id: 11,
    answer: "Yes",
    response_body:
      "We’ll add some of our wonderful actives that will minimize the appearance of any hyper or hypo-pigmentation.",
  });
  await db.answer.insert({
    order: 1,
    question_id: 11,
    answer: "No",
    response_body:
      "Great, we can eliminate the addition of unnecessary actives and only focus on those that you need.",
  });
  await db.answer.insert({
    order: 3,
    question_id: 12,
    ...
    response_body:
      "Make sure to wear proper SPF and we’ll make sure we add actives that are not sun-sensitizing.",
  });
  await db.answer.insert({
    order: 2,
    question_id: 12,
    ...
    response_header: "Noted.",
    response_body:
      "Make sure to wear proper SPF and we’ll make sure we add actives that will not make your skin extra sensitive to the sun during the day.",
  });
  await db.answer.insert({
    order: 1,
    question_id: 12,
    ...
    response_body:
      "We can focus on all the great actives your skin needs without the sun’s distraction – though we still recommend using a daily SPF.",
  });
  await db.answer.insert({
    order: 1,
    question_id: 13,
  ...
  await db.answer.insert({
    ...
    response_header: "Noted.",
    response_body:
      "We have amazing actives specifically for redness – keep going till the end to find out what we have chosen specifically for your needs.",
  });
  await db.answer.insert({
    order: 3,
    question_id: 13,
  ...
  await db.answer.insert({
    ...
    image_id: 26,
    response_body:
      "We have some wonderful actives that will minimize the appearance of lines. We’ll just focus on those that will give you the best bang for your buck.",
  });
  await db.answer.insert({
    order: 3,
    question_id: 15,
  ...
  await db.answer.insert({
    ...
    image_id: 30,
    response_header:
      "We’re going to add some of our luxurious anti-wrinkle actives. Keep going till the end to see what we recommend.",
  });
  await db.answer.insert({
    order: 7,
    question_id: 15,
  ...
  await db.answer.insert({
    ...
    response_body:
      "We’ll focus on the actives more appropriate to your skin and ensure we maintain your skin’s natural resilience.",
  });
  await db.answer.insert({
    order: 2,
    question_id: 16,
    answer: "Middle",
    response_body:
      "We’ll add some luxurious actives to your formulation that will help prevent and improve your skins firmness.",
  });
  await db.answer.insert({
    order: 3,
    question_id: 16,
    ...
    response_body:
      "We’ll add some of our wonderful actives that will improve your skin’s appearance of firmness.",
  });
  await db.answer.insert({
    order: 1,
    question_id: 17,
    ...
    response_body:
      "We can eliminate actives that will just be extra clutter for your wonderful skin.",
  });
  await db.answer.insert({
    order: 2,
    question_id: 17,
    answer: "Some",
    response_body:
      "We’ll keep this in mind (our algorithm’s mind) as we create your perfect formula.",
  });
  await db.answer.insert({
    order: 3,
    question_id: 17,
  ...
  await db.answer.insert({
    ...
    response_header: "Great!",
    response_body:
      "This means your skin is exposed to less pollution and better oxygenation!",
  });
  await db.answer.insert({
    order: 2,
    question_id: 22,
    answer: "Sometimes",
    response_body:
      "We’ll make sure to add extra actives to address the pollution your skin is exposed to.",
  });
  await db.answer.insert({
    order: 3,
    question_id: 22,
    ...
    response_body:
      "Your formula will include actives for anti-pollution and increased oxygenation to your skin.",
  });
  await db.answer.insert({
    order: 1,
    question_id: 23,
    ...
    response_header: "Lucky you!",
    response_body:
      "We’ll rather focus on areas that are of greater concern to you.",
  });
  await db.answer.insert({
    order: 2,
    question_id: 23,
  ...
  await db.answer.insert({
    ...
    response_body:
      "Stress can release cortisol and other factors that can negatively affect the skin. We’ll add some adaptogens and other actives that can help mitigate some of these effects.",
  });
  await db.answer.insert({
    order: 1,
    question_id: 24,
  ...
  await db.answer.insert({
    order: 2,
    question_id: 24,
    answer: "Flexitarian, I sometime eat meat",
    answer_value: 25,
  });
  await db.answer.insert({
    order: 3,
    question_id: 24,
    answer: "Pescatarian, I’ll have fish and seafood, but no meat",
    answer_value: 50,
  });
  await db.answer.insert({
    order: 4,
    question_id: 24,
    answer: "I stay away from red meat",
    answer_value: 75,
  });
  await db.answer.insert({
    order: 5,
    question_id: 24,
    answer: "I eat everything",
    answer_value: 100,
  });
  ...
  await db.answer.insert({
    ...
    image_id: 32,
    response_body:
      "We’ll add extra hydrating actives, but we recommend you drink more so your skin gets nourished from within as well.",
  });
  await db.answer.insert({
    order: 2,
    question_id: 27,
  ...
  await db.answer.insert({
    ...
    response_header: "Great!",
    response_body:
      "You don’t need extra actives to compensate from a lack of internal hydration.",
  });
  await db.answer.insert({ order: 1, question_id: 28, answer: "None" });
  await db.answer.insert({
    order: 2,
    question_id: 28,
    answer: "Full Coverage",
  });
  await db.answer.insert({
    order: 1,
    question_id: 29,
    answer: "Low",
    response_body:
      "We’ll add extra actives to give your skin an extra boost on oxygenation and nutrient delivery.",
  });
  await db.answer.insert({
    order: 2,
    question_id: 29,
  ...
  await db.answer.insert({
    ...
    response_header: "Great!",
    response_body:
      "We can eliminate the addition of unnecessary actives and only focus on those that you need. ",
  });
  await db.answer.insert({
    question_id: 30,
    order: 1,
    ...
    hide_answer: 0,
    response_body:
      "We'll add some of our wonderful actives that will minimize the appearance of hypopigmentation over time",
  });
