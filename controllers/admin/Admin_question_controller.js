...
app.get(
  ...
  async function (req, res, next) {
    ...
    const viewModel = new questionsAdminAddViewModel(
      db.question,
      "Add question",
      "",
      "",
  ...
  async function (req, res, next) {
    ...
    try {
      ...
      const data = await db.question.insert({
        ...
        extra_output_variable: extra_output_variable
          ? extra_output_variable
          : null,
        type,
      });
...
app.get(
  ...
  async function (req, res, next) {
    ...
    const viewModel = new questionsAdminEditViewModel(
      db.question,
      "Edit question",
      "",
      "",
    ...
    try {
      ...
      let lastQuestionId = await db.question
        .findAll({ order_by: ["id", "DESC"] })
        .then((data) => data[0].id);
      let found = false;
      ...
      if (exists.answers && exists.answers?.length > 0) {
        viewModel.lastOrderCount =
          exists.answers[exists.answers.length - 1].order;
      }
...
app.get(
  ...
  async function (req, res, next) {
    try {
      ...
      var viewModel = new questionsAdminDetailViewModel(
        db.question,
        "Question details",
        "",
        "",
